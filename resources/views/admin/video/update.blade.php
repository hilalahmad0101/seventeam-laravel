...
@section('content')
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
        <div>
            ...
            <h6 class="op-7 mb-2">Video - <a href="{{ route('admin.video.list') }}">List</a> - Update</h6>
        </div>
                                            ...
                                            <div class="row row-cards">
                                                <div class="col-sm-12 col-md-12">
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="text" class="form-control" name="title"
                                                            placeholder="Title" id="title" value="{{ $video->title }}">
                                                        @error('title')
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <select name="category_id" id="category_id" class="form-select">
                                                            ...
                                                            @foreach ($categories as $category)
                                                                <option @selected($category->id == $video->category_id)
                                                ...
                                                <div class="col-sm-12 col-md-12">
                                                    <div class="mb-3">
                                                        ...
                                                        <textarea name="description" class="form-control" id="description" cols="30" rows="10">{{ $video->description }}</textarea>
                                                        @error('description')
                                                            <span class="text-danger">{{ $message }}</span>
                                                        @enderror
                                                    </div>
                                                ...
                                                <div class="col-sm-12 col-md-12">
                                                    ...
                                                    @if ($video->thumbnail)
                                                        <img id="thumbnail"
                                                            src="{{ asset('images/video/thumbnail/' . $video->thumbnail) }}"
                                                            alt="" style="width:150px;margin-top:10px; ">
                                                    @endif
                                                </div>
    ...
    <script>
        ...
        uploadButton.addEventListener('click', async () => {
            ...
            const response = await fetch('/get-update-upload-progress', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    fileName: !file ? "{{ $video->file_name }}" : file.name,
                    id: {{ request()->id }},
                    "_token": "{{ csrf_token() }}"
                }),
            });
        ...
        async function uploadChunks() {
            ...
            while (currentChunk < totalChunks && uploading) {
                ...
                formData.append('video_id', {{ request()->id }});
                formData.append('_token', "{{ csrf_token() }}");

                await fetch('/update-upload-chunk', {
                    method: 'POST',
                    body: formData,
                });
        ...
        async function uploadUpdateChunks() {
            uploading = true;
